      ...
      {!showDetail &&
        <>
          {selectedAccount.map((account, index) => (
            <div className="flex pt-1 pb-1 mb-2 sm:pt-3 sm:pb-3 sm:bg-[#F5F7F8]"
              key={index}
            ...
            >
              <div key={index} className="flex items-center justify-between w-full px-7">
                <div className="flex items-center justify-center rounded-full sm:pl-6 sm:pr-6">
                  <img className="w-10 h-10 rounded-full sm:w-14 sm:h-14" src={account.logo.links.square} alt={account.name}
                  />
                  <div className="ml-3 font-semibold text-sm2 sm:text-2xl2 sm:font-medium">
                    {account.name}
                  </div>
                </div>
                <div>
